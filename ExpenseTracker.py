...
st.sidebar.markdown("---")
new_trip_name = st.sidebar.text_input("➕ สร้างEventใหม่:").strip()
if st.sidebar.button("สร้างEventใหม่"):
    if new_trip_name:
        try:
            conn = get_db_connection()
            conn.execute("INSERT INTO trips (name, status) VALUES (?, 0)", (new_trip_name,))
            conn.commit(); conn.close()
            st.success(f"✈️ สร้างEventใหม่ '{new_trip_name}' สำเร็จ!")
            time.sleep(1)
            st.rerun()
        except: st.sidebar.error("❌ ชื่อEventซ้ำ")
    ...
    if not deleted_trips:
        ...
        for dt in deleted_trips:
            ...
            if sub_c1.button("กู้คืน", key=f"res_{dt['id']}"):
                ...
                conn.commit()
                st.toast(f"🔄 กู้คืนEvent '{dt['name']}' เรียบร้อย!")
                time.sleep(1)
                st.rerun()
            if sub_c2.button("ลบ", key=f"pdel_{dt['id']}"):
                ...
                conn.commit()
                st.toast(f"💥 ลบEvent '{dt['name']}' ถาวรแล้ว!")
                time.sleep(1)
                st.rerun()
...
st.sidebar.markdown("---")
current_trip = st.sidebar.selectbox("🗺️ เลือกEvent:", active_trip_list)
trip_id = conn.execute("SELECT id FROM trips WHERE name = ? AND status = 0", (current_trip,)).fetchone()["id"]

# ====== ส่วนแก้ไขชื่อEventปัจจุบัน ======
with st.sidebar.expander("✏️ แก้ไขชื่อEventปัจจุบัน"):
    rename_input = st.text_input("เปลี่ยนชื่อEventเป็น:", value=current_trip).strip()
    if st.button("💾 ยืนยันเปลี่ยนชื่อ"):
        if rename_input and rename_input != current_trip:
            try:
                ...
                conn_rename.commit()
                conn_rename.close()
                st.success(f"✏️ เปลี่ยนชื่อEventเป็น '{rename_input}' เรียบร้อย!")
                time.sleep(1)
                st.rerun()
            except:
                st.error("❌ ชื่อEventนี้ซ้ำกับEventอื่นที่มีอยู่")
        elif not rename_input:
            st.error("⚠️ กรุณากรอกชื่อEvent")

if st.sidebar.button("🗑️ ย้ายEventลงถังขยะ"):
...
if st.sidebar.button("ดึงเข้าEvent"):
    if selected_u != "-- เลือก --":
        ...
        conn.commit()
        st.toast(f"➕ ดึง {selected_u} เข้าสู่Eventแล้ว!")
        time.sleep(1)
        st.rerun()
conn.close()
...
if not existing_members:
    st.title(f"✈️ ข้อมูลEvent: {current_trip}")
    st.warning("กรุณาเลือกสมาชิกเข้าEventก่อน")
    st.stop()
...
with tab3:
    ...
    if not expenses_rows: 
        ...
        if st.button("🎯 บันทึกปิดEvent", type="primary"):
            conn = get_db_connection()
